...
@Component
public class ERC20TransactionHandler extends AbstractTransactionHandler {
    ...
    public void parseTransactionInfo(TransactionReceipt transaction, TransactionInfo transactionInfo) {
        ...
        transactionInfo.setContractAddress(transaction.getTo());
        ...
        for (Log log : logs) {
            ...
            if (this.transferEventSignature().equals(eventSignature)) {
                ...
                transactionInfo.setData(new BigInteger(log.getData().substring(2), 16));
                break;
            }
        }
    }
